...
interface WorkerTask {
  ...
  resolve(response: VbReferencesWorkerResponse): void;
  reject(error: Error): void;
}

export class VbReferencesWorkerPool {
  ...
  resize(concurrency: number): void {
    ...
    while (this.slots.length > this.concurrency) {
      ...
      this.rejectSlotTask(slot, new Error("VBScript references worker was resized."));
  ...
  async close(): Promise<void> {
    ...
    for (const task of queued) {
      task.reject(new Error("VBScript references worker pool is closed."));
    }
    const slots = this.slots.splice(0);
    for (const slot of slots) {
      this.rejectSlotTask(slot, new Error("VBScript references worker pool is closed."));
    }
  ...
  private createSlotIfNeeded(): WorkerSlot | undefined {
    ...
    worker.on("message", (response: VbReferencesWorkerResponse) => {
    ...
    worker.on("exit", (code) => {
      if (code !== 0 && this.slots.includes(slot)) {
        this.replaceFailedSlot(slot, new Error(`VBScript references worker exited with ${code}.`));
      }
    });
...
function defaultWorkerPath(): string {
  return path.join(__dirname, "vb-references-worker.js");
}
