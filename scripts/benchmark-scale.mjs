...
<section class="panel"><div class="toolbar"><div><h2 style="margin:0">1x Backend Heatmap</h2><p class="footer" style="margin-top:4px">Median timing for the 1x sample. Hotter cells are slower on a log scale. Hover cells for exact values and TS/native ratio.</p></div></div><div class="heatmap-wrap"><table class="heatmap" id="oneXHeatmap"></table></div></section>
...
function renderHeatmap(){const columns=[["TypeScript","cold"],["TypeScript","hot"],["Native","cold"],["Native","hot"]],oneX=records.filter(r=>r.multiplier===1),values=oneX.map(r=>r.median).filter(Number.isFinite),min=Math.min(...values,0),max=Math.max(...values,1);let html="<thead><tr><th class='operation'>Operation</th>"+columns.map(c=>"<th>"+esc(c[0]+" "+c[1])+"</th>").join("")+"</tr></thead><tbody>";for(const operation of operations){html+="<tr><th class='operation'>"+esc(operation)+"</th>";for(const column of columns){const record=oneX.find(r=>r.backend===column[0]&&r.cacheMode===column[1]&&r.operation===operation);if(!record){html+="<td><div class='heatmap-cell missing'>-</div></td>";continue}const intensity=heatIntensity(record.median,min,max),ratio=backendRatio(record,oneX),tipText=record.backend+" "+record.cacheMode+" / "+operation+"\\n1x median "+fmt.format(record.median)+" ms\\nmin "+fmt.format(record.min)+" ms / max "+fmt.format(record.max)+" ms"+(ratio?"\\n"+ratio:"");html+="<td><div class='heatmap-cell' style='background:"+heatColor(intensity)+"' data-tip='"+attr(tipText)+"'><span class='time'>"+fmt.format(record.median)+"</span><span class='detail'>ms median</span></div></td>"}html+="</tr>"}document.getElementById("oneXHeatmap").innerHTML=html+"</tbody>";bind(document.querySelectorAll("#oneXHeatmap [data-tip]"))}
function heatIntensity(value,min,max){const low=Math.log10(min+1),high=Math.log10(max+1);return Math.max(0,Math.min(1,(Math.log10(value+1)-low)/Math.max(high-low,0.000001)))}
function heatColor(t){const cool=(0.50-t*0.22).toFixed(3),warm=(0.14+t*0.72).toFixed(3),gold=(0.08+t*0.24).toFixed(3);return"linear-gradient(135deg,rgba(56,189,248,"+cool+"),rgba(251,191,36,"+gold+") 52%,rgba(251,113,133,"+warm+"))"}
function backendRatio(record,oneX){const peerBackend=record.backend==="Native"?"TypeScript":"Native",peer=oneX.find(r=>r.backend===peerBackend&&r.cacheMode===record.cacheMode&&r.operation===record.operation);if(!peer||peer.median<=0)return"";return"vs "+peerBackend+" "+fmt.format(record.median/peer.median)+"x"}
